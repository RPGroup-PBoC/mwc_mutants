...
Ka = [100, 500]
Ki = [0.1, 5]
c, ep, ka, ki= np.meshgrid(c_0, epAI, Ka, Ki)
mut_arch = mut.thermo.SimpleRepression(R=R_0, effector_conc=c, ka=ka, ki=ki, 
                                   ep_ai=ep, ep_r=epRA_0, n_sites=n_sites) 
mut_fc = mut_arch.fold_change()
mut_params = {'effector_conc':c_0, 'ep_RA':epRA_0, 
               'R':R_0, 'n_sites':n_sites}

mut_delF = np.zeros((2, len(c_0)))
for i, ep in enumerate(epAI):
    mut_params['ep_AI'] = epAI[i]
    mut_params['Ka'] = Ka[i]
    mut_params['Ki'] = Ki[i]
...
ax[0].plot(c_0 / Ka_0, mut_fc[0, :, 0, 0], '-', color=colors[2],  label='mutant 1')
ax[0].plot(c_0[::50]/ Ka_0, mut_fc[0, ::50, 0, 0], 'o', color=colors[2], label='__nolegend__')
ax[0].plot(c_0/ Ka_0, mut_fc[1, :, 1, 1], '-', color=colors[4],  label='mutant 2')
ax[0].plot(c_0[::50] / Ka_0, mut_fc[1, ::50, 1, 1], 'o', color=colors[4], label='__nolegend__')
...
ax[0].legend(handlelength=1, loc='upper left')
fig.text(0.0, 1, '(a)', fontsize=9)
fig.text(0.5, 1, '(b)', fontsize=9)
plt.tight_layout()
# plt.savefig('thought_experiment_IND_kaki.pdf', bbox_inches='tight')gg