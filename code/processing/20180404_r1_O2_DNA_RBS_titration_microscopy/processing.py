...
noise_ims = [skimage.io.ImageCollection(f)[1] for f in noise_files]
avg_noise = mut.image.projection(noise_ims, mode='mean')
yfp_files = glob.glob('{}/{}_YFP_profile_*/*.tif'.format(data_dir, DATE))
yfp_ims = [skimage.io.ImageCollection(f)[1] for f in yfp_files]
...
for i, s in enumerate(samples):
    if ('camera_noise' not in s) and ('YFP_profile') not in s:
        try: 
            _, mutant, strain, IPTG, _ = s.split('/')[-2].split('_')
...
mean_auto = filt_df[filt_df['strain']=='autofluorescence']['mean_intensity'].mean()
