...
for filename in files:
    ...
    # Check that the data frame has at least 20K reads
    if len(dataframe.index) < 2E4:
...
for i in np.arange(len(df)):
    ...
    # Take the mean of the autos with intensities > 0
    auto = auto[auto > 0].mean()
...
for i in np.arange(len(df)):
    ...
    # Take the mean of the deltas with intensities > 0
    delta = delta[delta > 0].mean()
...
with open('output/' + str(date) + '_' + run + '_' + operator + '_' + \
          mutant_class + '_lacI_titration_MACSQuant.csv', 'w') as output:
