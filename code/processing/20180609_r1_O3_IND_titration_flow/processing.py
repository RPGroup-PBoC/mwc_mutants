...
# ###############################
# DON'T REUSE!!! THIS FILE REMOVES A SPURIOUS DATUM
# ###############################
...
# Load all files.
files = glob.glob('../../../data/flow/csv/{0}*.csv'.format(DATE))
...
for f in files:
    ...
    # Get the isix replicates of F164T in O3 dentifying information.
    ...
    samp_dict = dict(date=date, username=USERNAME, mutant=mutant,
                     ...
                     mean_FITC_H=mean_FITC, repressors=rep, run_no=run_no)
...
fold_change_df = fold_change_df[fold_change_df['IPTGuM'] != 500] 
# Save to a CSV.
