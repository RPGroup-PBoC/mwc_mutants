...
run = 'r1'
operator = 'O1O2O3'
mutant_class = 'other' # options dna/inducer/double/other
#==============================================================================
...
datadir = '../../../data/flow/csv/20170522/'

#  Glob the files of interest.
...
# Initialize pandas series to save the correct`ed YFP value
...
# Compute the pseudo-fold-change
for i in np.arange(len(df)):
    data = df.loc[i]
    delta = df[(df.IPTG_uM == data.IPTG_uM) &
               (df.strain == 'delta') & (df.operator==data.operator)].mean_YFP
    mean_fc = np.append(mean_fc, data.mean_YFP / delta)

# Convert the fold-change to a pandas DataFrame.
mean_fc = pd.Series(mean_fc)
mean_fc.name = 'pseudo_fold_change'
