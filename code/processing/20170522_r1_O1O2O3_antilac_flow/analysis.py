...
plt.figure()
for strain in ops:
    plt.plot(df[df.operator == strain].sort_values(by='IPTG_uM').IPTG_uM * 1E-6,
             df[df.operator == strain].sort_values(by='IPTG_uM').pseudo_fold_change,
             marker='o', linewidth=1, linestyle='--', label=strain)
plt.xscale('log')
plt.xlabel('IPTG (M)')
plt.ylabel('pseudo-fold-change')
plt.ylim([-0.01, 1.4])
plt.xlim([1E-8, 1E-2])
