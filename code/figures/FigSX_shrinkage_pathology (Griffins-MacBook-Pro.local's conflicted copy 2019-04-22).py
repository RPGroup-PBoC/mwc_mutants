...
ax[1].set_title(r'$K_A$, $K_I$, and $\Delta\varepsilon_{AI}$ modified', fontsize=8,
    backgroundcolor=colors['pale_yellow'], y=1.08)
...
for g, d in df.groupby('model'):
    ...
    _ax.plot(gt_s, gt_v, '.', color=colors['red'], 
             label='$K_A > K_I$', alpha=0.4)
    _ax.plot(lt_s, lt_v, '.', color=colors['blue'], 
             label='$K_A < K_I$', alpha=0.4)
for a in ax:
    a.legend(fontsize=8, ncol=2, loc='lower left')
plt.tight_layout()
plt.savefig('../../figures/FigSX_IND_shrinkage_pathology.pdf', bbox_inches='tight')