...
razo_data = pd.read_csv('../../data/csv/RazoMejia2018_data.csv')
razo_delF = pd.read_csv('../../data/csv/RazoMejia2018_delta_F_statistics.csv')
...
for g, d in razo_delF.groupby(['ref_bohr']):
    ...
    if label not in labels:
        ...
        label = '__nolegend__'
    ax[1, 0].plot(fc_mu['ref_bohr'], fc_mu['median'], '.', ms=3, color=color,
            label=label, 
            markeredgewidth=0.75)
    ax[1, 0].vlines(fc_mu['ref_bohr'], fc_mu['hpd_min'], fc_mu['hpd_max'], 
                lw=0.5, color=color, label='__nolegend__')
        
    ax[1, 1].plot(delF['ref_bohr'], delF['median'], '.', color=color, label=label,
        ms=2)
...
plt.savefig('../../figures/FigSX_deltaF_systematic_error.pdf', bbox_inches='tight')
