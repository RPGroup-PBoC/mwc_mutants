...
for g, d in DBL_pred.groupby('mutant'):
    ...
    ax0.errorbar(d['mode'], fc['mean'], fc['sem'], linestyle='none',
                lw=1, fmt='o', color=colors[g], label=g)
...
for g, d in DBL_pred.groupby('mutant'):
    ...
    _ = ax1.plot(pos[g] - 0.1, ssq_mode, 'o', markerfacecolor='w',
                markeredgecolor=colors[g], markeredgewidth=1, markersize=3,
                label='__nolegend__')
    _ = ax1.vlines(pos[g] - 0.1, ssq_min, ssq_max, lw=1, color=colors[g],
                  label='__nolegend__')
...
for i, m in enumerate(data['mutant'].unique()):
    ...
    _ = ax1.plot(pos[m] + 0.1, ssq_median, 'D', markerfacecolor='w',
                markeredgecolor=colors[m], markeredgewidth=1, markersize=3, label='__nolegend__')
    _ = ax1.vlines(pos[m] + 0.1, ssq_min, ssq_max, lw=1, color=colors[m], label='__nolegend__')

for m, i in pos.items():
    if i%2 == 0:
        _ = ax1.vlines(i, -1, 50, color=pboc['pale_yellow'], lw=25, zorder=-1)
ax0.legend(fontsize=7, handletextpad=0.1)
ax1.plot([], [], 'o', markerfacecolor='w', markeredgecolor='k', markersize=3, markeredgewidth=1,
        label='predicted')
ax1.plot([], [], 'D', markerfacecolor='w', markeredgecolor='k', markersize=3, markeredgewidth=1,
        label='measured')
