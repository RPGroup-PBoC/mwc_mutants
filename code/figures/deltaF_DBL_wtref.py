...
for d in tqdm(DBL['mutant'].unique()):
    ...
    _df = pd.DataFrame(np.array([c_range, cred_region[0, :], cred_region[1, :]]).T,
                       columns=['IPTGuM', 'bohr_min', 'bohr_max'])
    _df['mutant'] = d
    draw_dfs.append(_df)
...
for g, d in DBL.groupby(['mutant']):
    ...
    _ = _ax.fill_between(c_range, wt_bohr - mutant['bohr_max'],
                        wt_bohr - mutant['bohr_min'], color=colors[g], alpha=0.5)
...
backgroundcolor='#E3DBD0')
plt.savefig('../../data/csv/Fig7_DBL_deltaF.pdf', bbox_inches='tight')