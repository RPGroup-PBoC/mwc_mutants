...
fc_dfs = []
print(df.head())
mean_auto = pd.DataFrame(df[df['mutant']=='auto'].groupby('IPTGuM')['mean_FITC_H'].mean()).reset_index()

grouped = df[df['operator'] != '--'].groupby(['IPTGuM', 'operator'])
for g, d in grouped:
    mean_delta = d.loc[d['mutant'] == 'delta']['mean_FITC_H'].values[0]
    _auto = mean_auto[mean_auto['IPTGuM']==g[0]]['mean_FITC_H'].values[0]
    d['fold_change'] = (d['mean_FITC_H'] - _auto) / \
        (mean_delta - _auto)
    fc_dfs.append(d)
